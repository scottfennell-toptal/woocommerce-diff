...
foreach ($items as $item_id => $item) :
    if (apply_filters('woocommerce_order_item_visible', true, $item)) {
        ...
        $author_id = $wpdb->get_var("SELECT `post_author` FROM `wp_posts` WHERE `ID`='" . $item['product_id'] . "'");
        //echo "--".$product->get_id()."--";
...
//                }
                ...
                if ($show_download_links && !$sent_to_admin) {
                ...
                <ul>
                    <?php
                    if ($dokan_profile_settings["support_link"] && !$sent_to_admin) {
                        ?>
                        <li><a href="<?php echo $dokan_profile_settings["support_link"]; ?>" target="_blank">Support</a></li>
                    <?php } ?>
<!--                    <li><a href="<?php echo get_the_permalink($item['product_id']) . "/?leave_review=1"; ?>" target="_blank">Leave a review</a></li>-->
                </ul>
            </td>
